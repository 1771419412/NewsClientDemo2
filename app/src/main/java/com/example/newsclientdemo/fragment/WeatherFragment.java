...
import com.example.newsclientdemo.gson.Forecast;
import com.example.newsclientdemo.gson.Weather;
import com.example.newsclientdemo.service.AutoUpdateService;
import com.example.newsclientdemo.util.HttpUtil;
import com.example.newsclientdemo.util.Utility;
        ...
        HttpUtil.sendOkHttpRequest(weatherUrl, new Callback() {
                ...
                getActivity().runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (weather != null && "ok".equals(weather.status)) {
                            ...
                            Toast.makeText(getActivity(), "获取天气信息失败1", Toast.LENGTH_SHORT).show();
                        }
            ...
            public void onFailure(Call call, IOException e) {
                ...
                getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(getActivity(), "获取天气信息失败2", Toast.LENGTH_SHORT).show();
                        swipeRefresh.setRefreshing(false);
                    }
                });
            }
